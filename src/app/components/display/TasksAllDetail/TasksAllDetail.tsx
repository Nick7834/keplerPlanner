...
import styles from './TasksAllDetail.module.scss'
...
import { foldersAll, setTaskCheck, setTaskId } from '@/redux/slices/tasks';
...
export const TaskAllDetail = ({taskDefault}: any) => {
  ...
  useEffect(() => {
    if(data && taskDefault ) {
      const nowDone = taskDefault.find((task: any) => task._id === data?._id);
  ...
  useEffect(() => {
    if (taskCheck && taskIdSatate && taskDefault?.tasks) {
    ...
    if (taskDefault && taskDefault.length > 0 && (!data || !taskDefault.find((task: any) => task?._id === data?._id))) {
      const filterTaskOne = taskDefault.filter((task: any) => !task?.done);
      const filterTaskOneDone = taskDefault.filter((task: any) => task?.done);
  ...
  }, [taskDefault, data, taskCheck, taskIdSatate]);
  ...
  const delTaskNow = async (id: any) => {
    try {
      ...
      if (taskDefault && taskDefault) {
        const tasks = taskDefault;
